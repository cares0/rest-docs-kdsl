...
import io.github.cares0.restdocskdsl.annotation.RequestCookieDocs
...
class RequestCookieDocsAnnotationElementResolver(
    private val logger: KSPLogger,
) : FunctionResolver {

    override fun isSupport(kSFunction: KSFunctionDeclaration): Boolean {
        return kSFunction.containsAnnotation(RequestCookieDocs::class) ?: false
    }
    ...
    override fun resolve(kSFunction: KSFunctionDeclaration): List<HandlerElement> {
        ...
        kSFunction.annotations.forEach { annotation ->
            if (annotation.annotationType.getQualifiedName() != RequestCookieDocs::class.qualifiedName) return@forEach
            ...
            (names + values).forEach { extractedElements.add(RequestCookieElement(it)) }
        }
